...
from .dependencies import Conv2dNormActivation
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def forward(
        ...
        losses = {}
        if self.training:
            # print("RPN TRAINING HAPPENING NOW!!")
            
            if targets is None:
                raise ValueError("targets should not be None")
            labels, matched_gt_boxes = self.assign_targets_to_anchors(anchors, targets)
            # labels_np = labels[0].cpu().numpy()
            # pos_indices = np.where(labels_np == 1)[0][:10]  # First 10 positive anchors
            # neg_indices = np.where(labels_np == 0)[0][:10]
            # print(pos_indices)
            # print(neg_indices)
